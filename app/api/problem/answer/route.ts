...
import { isArkEnabled } from '@/lib/server/ark-agents';
...
export async function POST(request: Request) {
  ...
  if (!isArkEnabled()) await waitMock(700);
  const a1 = await submitAnswer(parsed.data.conversationId, parsed.data.round, parsed.data.inputMode, parsed.data.text);
