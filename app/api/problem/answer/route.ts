...
export async function POST(request: Request) {
  ...
  if (!isFastAIEnabled()) await waitMock(700);
  const tenant = await resolveTenant();
  const a1 = await submitAnswer(parsed.data.conversationId, tenant, parsed.data.round, parsed.data.inputMode, parsed.data.text);
