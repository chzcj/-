...
const PLACEHOLDER = /测试画像|生命周期测试|从服务器记录恢复|家庭画像已从服务器记录恢复|继续交流即可补充细节/i
function isPlaceholderCore(text: string): boolean {
  const t = text.trim()
  if (!t) return true
  if (PLACEHOLDER.test(t)) return true
  // 全由占位短语组成也判空
  if (t.replace(PLACEHOLDER, '').replace(/[，,。：:\s]/g, '').length === 0) return true
  return false
}
...
export async function GET(request: Request) {
  ...
  return ok({
    ...
    portraitCards: uiSnapshot?.portraitCards,
