...
function GeneratingPageContent() {
  ...
  useEffect(() => {
    ...
    apiClient.generateUnderstandingCard(conversationId, { signal: controller.signal }).then((result) => {
      if (!mounted) return;
      if (!result.ok) {
  ...
  return (
    <AppShell>
      <div className="page without-voice">
        ...
        {!conversationId ? (
          <ErrorState title="这张理解卡没有顺利生成出来" description="可以回到上一页重新整理。" primaryLabel="返回上一页" onPrimary={() => router.back()} />
        ) : error ? (
