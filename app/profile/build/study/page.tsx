...
import { createEntryRecord, markEntryCompleted } from '@/lib/storage/entryStorage'
...
export default function StudyInputPage() {
  ...
  const config = getEntryConfig('study')

  async function handleSubmit(text: string, _mode: InputMode) {
    if (!text.trim() || loading) return
    setLoading(true)
    createEntryRecord({ entryType: 'study', rawText: text.trim() })
    markEntryCompleted('study')
    setLoading(false)
    router.push('/profile/build/study/follow-up')
  }

  return (
    <AppShell>
      <div className="page capture-page with-capture-dock">
        <PageHeader title={`学习与作业 ${config.stepLabel}`} showBack onBack={() => router.push('/profile/build')} />
