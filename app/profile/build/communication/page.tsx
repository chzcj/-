...
export default function CommunicationInputPage() {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const config = getEntryConfig('communication')
  async function handleSubmit(text: string, _mode: InputMode) {
    if (!text.trim() || loading) return
    setLoading(true)
    createEntryRecord({ entryType: 'communication', rawText: text.trim() })
    setLoading(false)
    router.push('/profile/build/communication/follow-up')
  }
  return (
    <AppShell>
