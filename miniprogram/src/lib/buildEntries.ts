...
export function getNextBuildEntry(
  entryType: BuildEntryType
): BuildEntryType | 'final-follow-up' | null {
  const index = BUILD_ENTRY_ORDER.indexOf(entryType)
...
export function mpFollowUpPath(entryType: BuildEntryType): string {
  return `/packageOnboarding/pages/follow-up/index?entryType=${entryType}`
}

export function mpSummaryPath(entryType: BuildEntryType): string {
  return `/packageOnboarding/pages/summary/index?entryType=${entryType}`
}

export function firstBuildEntryPath(): string {
  return mpCapturePath(BUILD_ENTRY_ORDER[0])
}
