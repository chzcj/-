...
### HiFiBuildShell（五入口 Onboarding）

- 路由：`/profile/build/*`
- 样式：`app/profile/build/hifi-build.css`
- 保留采集逻辑，只换 hi-fi 皮；`BuildFlowGuard` / `OnboardingGuard` 控制解锁四 Tab
...
| 元素 | 类名 / 组件 | 说明 |
|------|-------------|------|
| 空态引导 | `hero-card` + `suggestion-strip` | 短文案 + 可点 chips |
| 用户气泡 | `message-row.user` | 右对齐 |
| AI 气泡 | `message-row.ai` + `bubble` | 左对齐，主体为 AI 输出 |
| Thinking | `thinking-bubble` / 四宫格 chips | 首字前占位，不假装已想好 |
| Section | `bubble-section` + `section-label` | prose 后的结构化展开 |
| Actions | action 条 | sections 完成后出现，解锁输入 |
| 深度展开 | understanding-card | hidden section 点开即读 |

### 画像 · `/family-profile`

- `soft-card`、`authority-insight-card`（清北学霸 · 家庭智慧背书气质）
- `StructuralTensionCard`：机制张力展示，人话、非诊断标签
...
与 [`PRODUCT.md`](PRODUCT.md) 流式原则一致：

1. **thinking 先于 prose**：四宫格让家长知道系统在「读你家孩子」
2. **prose 增量流出**：无重复累加、结束不缩字闪烁；客户端 `smoothQueue` 可略平滑，不得掩盖内容错误
3. **section 紧接 prose**：单次 LLM marker 流，感知为「一段话 + 结构化展开」
4. **actions 后解锁输入**：不必等 hidden/final
5. **可打断**：abort 后保留已展示内容
...
- 交流：流式打断、section 重试、深度展开、`/daily/how-to-speak`、action 条
- 语音：全屏 `recording-mask`
- 预演：4 轮检查点 modal、system hint
- 任务：反馈顶栏 + `task-submit-dock`
- 画像：accordion、内联账号编辑
- 采集：`entry-row` hub、`HiFiBuildHero`、`BuildFlowGuard`
