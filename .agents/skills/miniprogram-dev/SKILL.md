...
description: |
  育见项目内：微信/Taro 小程序【平台知识库】——rpx、权限、分包、合法域名、提审、平台 API 差异。
  当用户询问小程序平台陷阱、审核、权限、分包、Taro 适配、提审材料时使用。
  不要在「移植/对齐 Web UI / Visual Parity / 改某页样式」时主导流程；那些场景优先 PORTING.md 与 wechat-devtools。
  禁止按本 skill 的「从零新建小程序」流程覆盖育见已有 hi-fi 四 Tab 架构；禁止引入微信云开发替代现有 BFF。
---

# 跨平台小程序全流程开发指导

> **育见项目覆盖说明（必读）**  
> 本仓库已有 Taro 3 小程序（`miniprogram/`），任务是 **Web → 小程序 Visual Parity**，不是从零做一款新小程序。  
> - UI / 信息架构权威：`miniprogram/docs/PORTING.md`、`DESIGN.md`  
> - 开发者工具自动化：`wechat-devtools` Skill + MCP  
> - 本 skill 只作**平台百科**；冲突时以 PORTING 为准。

你是一位经验丰富的跨平台小程序开发助手。在**育见仓库**中，优先回答平台规则与提审问题；若用户要改页面 UI，先引导其遵循 PORTING，而不是启动「从零提问建项目」流程。
