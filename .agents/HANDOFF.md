...
## 2026-07-23 02:25 | Claude | 深度建模 v4.1：harness 硬化 + 语义激活 + 反向索引

**做了什么**
- P0 harness 接电：`harness/confidence-clamp.ts`（atom confidence 按 evidenceTier×认识论双上限钳制，inferred/hypothesized 强制非高价值不进向量池）；modelReview 接入 `bayesian.ts` computeLikelihood（weight 由 posterior 映射，有反证必 weakened）；`harness/background-post-gate.ts`（机制矩阵/dossier 落库前校验，结构性违规带反馈重试一次）
- 理论卡 20 张全 rich 9 字段（7 张 legacy + 3 张半成品补齐，legacyCard 机制删除；SP "15×9"→"20×9"）
- 机制 schema 扩展：`sceneActivations`（场景×角色×强度）+ `relatedMechanisms` 关系边（competesWith 等五种，post-gate 剔除悬空边）；theoryMatches.matchedFactIds 确定性回填 mechanism.evidenceRefs
- 语义激活切片：`dossier-semantic-slicer.ts` + digest-store 组件 embedding 预计算（独立 layer item，版本对齐，全链回退正则）；缓存复用时按新 query 重切 dossierSlice
- atom→机制反向索引：`fact_atoms.supported_mechanism_names` 列（自动迁移）+ `atom-mechanism-links.ts`（复核后 embedding 匹配写回，距离≤0.6，整族替换）；router 将支撑主机制的 atom 排到 domainAtomFacts 前列
- analysis prose 预算按材料厚度 240/360 分档（hasDeepMaterial）；前台 SP 补 domainAtomFacts 纪律 + 小型重推理指引；机制卡携带证据引用/激活场景/机制关系

**为什么**
- 审计发现 v4 三根硬支柱（三角验证/贝叶斯池/后台 post-gate）停在提示词自觉层面，bayesian.ts 是死代码；机制激活靠 3 个正则；机制间无关系无法表达多机制共同驱动。审计报告见项目外 00-文件资料/架构文件/深度建模机制审计与改进意见.md

**验证**
- `npm run typecheck` ✓（src/app 零错误）
- `npx tsx scripts/test-harness-gates.mjs` 28/28 ✓（钳制/gate/贝叶斯纯函数契约）
- `npx tsx scripts/test-atom-mechanism-links.mjs` 11/11 ✓（临时 pgvector 容器，schema 迁移/整族替换/检索携带链接）

**下一步**
- 服务器跑 `npx tsx scripts/run-portrait-v3-smoke.mjs <familyId> <childId>` 触发深度复核，验收 sceneActivations/关系边/component_embeddings/supported_mechanism_names 实际生成
- 存量 inferred 高价值 atom 未清理（用户决定跳过）

**风险/冲突**
- 新字段需下一次 deep_mechanism_review 全量跑完才有数据；组件 embedding 缺失时语义切片自动回退正则，无行为风险
- 本机 .env.local 无 FAST_AI key（与 README"本地变量完整"描述不符），LLM 链路只能服务器验证
